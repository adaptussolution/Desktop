...
{-----------------------------------------------------------------------}
{
  ...
  TRVDrawLines = class (TList)
    ...
    public
      ...
      function GetString(Index: Integer; AItems: TStringList): String;
      function GetSubString(Index: Integer; AItems: TStringList;
        AStartIndex, ALength: Integer): String;
      function GetRightString(Index: Integer; AItems: TStringList;
...
function TRVDrawLines.GetString(Index: Integer; AItems: TStringList): String;
begin
 {$IFDEF RVDONOTUSEUNICODE}
  with Items[Index] do
    Result := Copy(AItems[ItemNo], Offs, Length);
  {$ELSE}
  with Items[Index] do
    Result := RVU_Copy(AItems[ItemNo], Offs, Length,
      TCustomRVItemInfo(AItems.Objects[ItemNo]).ItemOptions);
  {$ENDIF}
end;
{------------------------------------------------------------------------------}
{ The same, but returns substring of drawitem's text, starting at AStartIndex
  position (1-based).                                                          }
function TRVDrawLines.GetRightString(Index: Integer; AItems: TStringList;
  AStartIndex: Integer): String;
begin
  {$IFDEF RVDONOTUSEUNICODE}
  with Items[Index] do
    Result := Copy(AItems[ItemNo], Offs+AStartIndex-1, Length-AStartIndex+1);
  {$ELSE}
  with Items[Index] do
    Result := RVU_Copy(AItems[ItemNo], Offs+AStartIndex-1, Length-AStartIndex+1,
      TCustomRVItemInfo(AItems.Objects[ItemNo]).ItemOptions);
  {$ENDIF}
end;
{------------------------------------------------------------------------------}
...
function TRVDrawLines.GetSubString(Index: Integer; AItems: TStringList;
  AStartIndex, ALength: Integer): String;
begin
  with Items[Index] do begin
    ...
    {$IFDEF RVDONOTUSEUNICODE}
    Result := Copy(AItems[ItemNo], Offs+AStartIndex-1, ALength);
    {$ELSE}
    Result := RVU_Copy(AItems[ItemNo], Offs+AStartIndex-1, ALength,
      TCustomRVItemInfo(AItems.Objects[ItemNo]).ItemOptions);
    {$ENDIF}
  end;
end;
