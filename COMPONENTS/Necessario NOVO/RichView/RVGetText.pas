...
interface
uses RVItem, RichView, RVEdit, CRVData;
...
begin
  if RVData.GetItemStyle(ItemNo)>=0 then
    ...
    Result := RVData.GetItem(ItemNo).AsText(0, RVData, RVData.GetItemText(ItemNo), '', True, False)
  else
    Result := '';
end;
...
begin
  ...
  while (Last<=Len) do begin
    if Pos(s[Last], rve.Delimiters)>0 then
      break;
    inc(Last);
  end;
  dec(First);
  while (First>0) do begin
    if Pos(s[First], rve.Delimiters)>0 then begin
      inc(First);
      break;
    end;
    dec(First);
