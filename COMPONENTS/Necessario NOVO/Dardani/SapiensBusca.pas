...
type
  ...
  public
    ...
    function GetSelection(Delimiter: Char = #0): WideString;
  published
  ...
  public
    ...
    destructor Destroy; override;
    function GetFieldByName(sFieldName: String): TEmbQueryField;
...
function TEmbQueryField.GetValues: TStrings;
begin
	Result := FValues;
end;
...
procedure TEmbQueryField.SetValues(Value: TStrings);
begin
...
function TEmbQuery.GetFieldByName(sFieldName: String): TEmbQueryField;
var
	i : Integer;
  Retorno : TEmbQueryField;
begin
	Retorno := nil;
	for i := 0 to Self.Fields.Count -1 do
  begin
		if UpperCase(Self.Fields[i].FInternalName) = UpperCase(sFieldName) then
    begin
    	Retorno := Self.Fields[i];
      Break;
    end;
  end;
  Result := Retorno;
...
function TEmbQueryField.GetSelection(Delimiter: Char): WideString;
var
	i : Integer;
	C : String;
  sResult: String;
...
//  Result := Values.DelimitedText;
  ...
  sResult := '';
	...
	for i := 0 to Self.FValues.Count -1 do
  	sResult := sResult + C + Self.FValues[i] + C + ', ';
    ...
    begin
      ...
      begin
        ...
        begin
					...
					F := FEmbQuery.GetFieldByName(Cds.Fields[i].FieldName);
      		if F <> nil then
        		F.Values.Add(FieldByName(sNomeCampo).AsString);
        end;
